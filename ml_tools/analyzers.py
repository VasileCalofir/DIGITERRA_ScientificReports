    ...
    def set_performances_on_test_data(self):
        ...
        for indicator, function in INDICATOR_FUNCTION_MAP.items():
            test_perf = Performance(self.simulation,
                                    round(function(self.trainer.y_test,
                                          self.predictor.prediction_on_test), 2),
    ...
    def set_performances_on_train_data(self):
        ...
        for indicator, function in INDICATOR_FUNCTION_MAP.items():
            train_perf = Performance(self.simulation,
                                     round(function(self.trainer.y_train,
                                           self.predictor.prediction_on_train), 2),
                                     self.trainer.trained_model,  # type: ignore
                                     indicator,
                                     data_type="train")
...
class Writer:
    ...
    def __init__(self, config: Config, simulation: str, performances: List[Performance], results_file_name: str):
        ...
        self.header = ["Simulation", "Description", "Features",
                       "Hyper-params", 'Model', 'Data type', 'Indicator', 'Value']
    ...
    def prepare_data_for_writing(self):
        ...
        for performance in self.performances:
            ...
            features = ", ".join(
                self.config.get_features(self.simulation_name))
            hyper_params = json.dumps(
                self.config.get_hyper_parameters(self.simulation_name))
            row = [self.simulation_name, description,
                   features, hyper_params] + performance_data
