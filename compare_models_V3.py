...
def create_simulations_config_file(features_set):
    ...
    for model in models:
        options = ["no_hyper_params"] if "regression" in model else [
            "no_hyper_params", "with_hyper_params_optimisation"]

        for option in options:

            current_simulation = create_simulation(
                base_simulation, features, model, option, models_config)
    ...
    for features_set, features in best_features_by_set.items():
        ...
        for simulation in simulations:
            ...
            if "min-max scale" in scaler_options:
                trainer = Trainer(x, y, model_type,
                                  hyper_params, use_min_max_scale=True)
            else:
            ...
            model_filename = f"{models_directory}/{model_type.replace(' ', '_')}_{option.replace(' ', '_')}_{features_set}.pkl"
            with open(model_filename, "wb") as model_file:
                pickle.dump(trainer.trained_model, model_file)

            # Save the scaler (if used) to a pickle file
            if trainer.scaler:
                scaler_filename = f"{models_directory}/scaler_{model_type.replace(' ', '_')}_{option.replace(' ', '_')}_{features_set}.pkl"
                with open(scaler_filename, "wb") as scaler_file:
                    pickle.dump(trainer.scaler, scaler_file)
            ...
            writer = Writer(config, simulation,
                            reader.performances, results_file_name)
            writer.append_to_csv()

            # Collecting performance data for plotting
            pareto_data_sets["R2 vs. MAE"]["R2"].append(
                get_value_for_pareto_plot(reader, "R2", "test"))
            pareto_data_sets["R2 vs. MAE"]["MAE"].append(
                get_value_for_pareto_plot(reader, "MAE", "test"))
            pareto_data_sets["R2 vs. FUR"]["R2"].append(
                get_value_for_pareto_plot(reader, "R2", "test"))
            pareto_data_sets["R2 vs. FUR"]["FUR"].append(
                len(features)/ALL_FEATURES_COUNT)
            pareto_data_sets["MAE vs. FUR"]["MAE"].append(
                get_value_for_pareto_plot(reader, "MAE", "test"))
            pareto_data_sets["MAE vs. FUR"]["FUR"].append(
                len(features)/ALL_FEATURES_COUNT)
            ...
            plotter = RealPredictedPlotter(
                config, "figs", True, simulation, features_set, features, trainer, predictor)
